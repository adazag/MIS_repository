CREATE VIEW data_out_fc.for_salaries AS
with
...
SELECT 
  ...
  MTH_NAME,
  case when PROJ_ID in (-11, -9, -8, -7, -6, -5, -4, -3, -2, -1, 4, 7) then TIME_DAY_AMT else 0 end SickLeave,
  case when PROJ_NAME not like 'Non-working%' or PROJ_ID in (-13, -10) then TIME_DAY_AMT*MULTR_RATE else 0 end Actuals
FROM data_in_sulu_v1.[sulu_actl_vw] a
),
...
GRANT SELECT
    ON OBJECT::[data_out_fc].[for_salaries] TO [data_out_fc_read_all]
    AS [dbo];
GO
