...
CREATE PROCEDURE [data_mis_project].[update_revenue_fixed] as

IF OBJECT_ID('[data_mis_project].[revenue_fixed]', 'U') IS NOT NULL TRUNCATE TABLE [data_mis_project].[revenue_fixed];
...
--,CASE WHEN DBL_CNTNG_IND='Y' THEN PARNT_FIN_PROJ_ID ELSE PROJ_ID END AS NEW_PROJ_ID
--FROM PROJECTS
--WHERE NEW_PROJ_END_DATE>@proj_end_date
--),
-------------------------------
/*Currency_rates_fixed*/
-----------------------------
with 
CRNCY_FIX_RATE AS (
SELECT
...
,fund_amount as AMT_BEFORE_DISC
,0 as AMT_DISC
...
,AMT_DISC
,AMT_AFTER_DISC
,b.exchange_rate as CRNCY_RATE
,TYPE
from Finance a
left join CRNCY_FIX_RATE b 
ON a.CRNCY_CODE = b.CRNCY_CODE
	AND a.CRNCY_DAY_DATE >= b.FX_CRNCY_START_DATE AND a.CRNCY_DAY_DATE <= b.FX_CRNCY_END_DATE
...
insert into [data_mis_project].[revenue_fixed]
select *
from FINANCE_PLN
--where PROJ_ID=8881
GO
...
GRANT EXECUTE
    ON OBJECT::[data_mis_project].[update_revenue_fixed] TO [lingaro-mis-adf]
    AS [dbo];
GO
