CREATE PROCEDURE [data_mis_project].[update_attrition] as

IF OBJECT_ID('[data_mis_project].[attrition]', 'U') IS NOT NULL TRUNCATE TABLE [data_mis_project].[attrition];

-- Define start variables for the calendar generation
DECLARE @MinDate DATE = '2023-01-01';
DECLARE @CurentDate DATE=GETDATE()
DECLARE @MaxDate DATE = EOMONTH(DATEADD(MONTH, 6, GETDATE()));  -- generating dates for the next 6 months from today
...
select  employee_id
		...
		WHEN oe.leave_ind = 1 and end_date_in_org_unit <> EOMONTH(end_date_in_org_unit) THEN EOMONTH(end_date_in_org_unit)
...
first_contract_start_date as (
select	 employee_id
		,MIN(contract_start_date) as first_contract_start_date
...
employee_contract AS (
	...
	LEFT JOIN employee_organization_unit_last_changes oeou ON ipot.employee_id = oeou.employee_id AND (oeou.start_date_in_org_unit <= dl.DayDate AND to_this_org_unit_end_date >= dl.DayDate)
	...
	left join first_contract_start_date  fcsd on ipot.employee_id = fcsd.employee_id
),
...
--left join data_in_sulu_v1.sulu_empee_lkp emp on a.employee_id = emp.EMPEE_ID
...
SELECT
	...
	,c.month_cnt
	,CAST(ROUND(((fte / 100) / month_cnt),4) as numeric (6,4)) as capacity
...
WHERE DayDate = EOMONTH(DayDate))

insert into [data_mis_project].[attrition] 
