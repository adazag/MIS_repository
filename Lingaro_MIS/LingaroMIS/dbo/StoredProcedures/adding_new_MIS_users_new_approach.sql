CREATE PROCEDURE [dbo].[adding_new_MIS_users_new_approach] AS 
...
BEGIN
	...
	FROM [data_in].[sec_emp_application_role] s
	LEFT JOIN [data_mis_project].[org_emp] e ON s.employee_id = e.id
	WHERe 1=1
	AND (s.role_name = 'HR' OR s.role_name = 'HR_STAFF' OR s.role_name = 'SUPER_HR')
	AND e.active_ind = 1 
	AND email NOT IN (SELECT name FROM sysusers WHERE islogin = 1 AND issqluser = 0)
	AND (leaver_last_day_of_work > GETDATE() or leaver_last_day_of_work is null)
...
BEGIN
	...
	LEFT JOIN [data_mis_project].[org_emp] e ON s.employee_id = e.id
	WHERe 1=1
	AND (s.role_name = 'IT' OR s.role_name = 'SECURITY')
	AND e.active_ind = 1 
	--AND email NOT IN (SELECT name FROM sysusers WHERE islogin = 1 AND issqluser = 0)
	AND (leaver_last_day_of_work > GETDATE() or leaver_last_day_of_work is null)
